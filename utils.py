...
def cryotiger(tsPath, outPath, modelPath, timesToInterpolate):
    ...
    generateOutputMRCTiltSeries(dstPath, tsPath)
...
def interpolate(pngPath, outPath, pathToModel, timesToInterpolate):
    ...
    rmPath = os.path.join(pngPath, fnInterp+'_cryoTIGER_model')
    # Move interpolated frames generated by the cryoTIGER model to a specific folder
    if os.path.exists(rmPath):
        shutil.rmtree(rmPath)  # Remove existing folder if it exists
...
def generateOutputMRCTiltSeries(outPath, tsFile, suffix=''):
    # Process interpolated frames for the cryoTIGER model
    folderPath, fn = os.path.split(tsFile)
    fnNoExt, _ = os.path.splitext(fn)
    mrcFn = os.path.join(folderPath, fnNoExt+suffix+'_interpolated.mrc')
    print('mrcFn %s ' % mrcFn)
    process_interpolated_frames(outPath, mrcFn)
